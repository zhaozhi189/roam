...
| v0.3 | 2026-07-15 | **v3 推进规则修正**(zhi 实拍反馈「时间太短,还没拍完就结束了」)。根因两条:① 覆盖段 maxSec 硬切(orbit 45s / up/down 30s)——语音让「缓慢转一圈」,转得慢就被拦腰截断;② 陀螺仪降级路径直接按最短秒数走(总 39s)根本拍不完。修正:覆盖段废弃 maxSec,改**停滞检测**——覆盖达标才换段;没达标但环还在点亮新扇区(=人还在转)就一直等;连续 12s(SCAN_REC_STALL_SEC)没新扇区(卡住/俯仰一直不达标)才兜底推进,120s(SCAN_REC_STEP_CAP_SEC)防挂死。降级路径新增 noGyroSec 按慢速转一圈标定(orbit 40s / up、down 各 25s,总 98s)。node 仿真验证三场景:慢转 64s 不被截断、卡住 12s 兜底、降级 98s 完整走完。**真机复验通过(2026-07-15,zhi 实拍)**:① 段 23s 时 97% 未被切、覆盖满才进 ② 段、天花板打勾正常;修复前后同房间对比 45.05s(正好卡 v2 orbit 45s maxSec 特征值)→ 63.5s(转满为准),录制完成自动保存相册链路 OK。v2 的语音播报/覆盖环/俯仰打勾也在本次实拍确认可用 |
| v0.4 | 2026-07-15 | **v4 推进权交给拍摄者**(zhi v3 复验后反馈「还是来不及,小平米都拍不完整,大房子更甚」)。认知升级:方位环转满一圈 ≠ 房间拍完整(大房间要走位/补细节),**任何自动推进模型都会误判节奏** → 引导段(orbit/up/down)永不自动推进,新增「下一段 ▸」按钮(段开始 3s 后出现),点了才走。完成度降级为纯提示:覆盖达标 → 按钮变绿「✓ 下一段」+ 语音「拍够了可以点下一段」+ 短震;真停滞(10s 无新扇区,认真转圈不打扰)才语音提醒一次。v3 的停滞自动兜底废弃(专注拍细节会被误判成卡住)。ready/loop 段仍按秒自动。新增总录制上限 5 分钟(webm→base64 过桥内存安全线),280s 语音预警、300s 自动保存;大房子分多次拍,素材累积 + `scan-train.sh --append`(ADR-015)并入。node 仿真:orbit 停 116s 不自动切、手动推进链路、卡住只提醒不推进、封顶自动保存全过;**待真机复验** |
| v0.5 | 2026-07-19 | **v4 真机复验通过**(WiFi adb 远程验证 + zhi 实拍):① 段「下一段 ▸」按钮 3s 后出现、静置不自动推进(v3 停滞兜底确认废弃);覆盖环/俯仰打勾正常;手动推进 ①→④ 全链路走通;④ 收尾段按钮正确隐藏、按秒自动、录完自动保存 `roam-scan-*.webm`(20MB/1.5min)。补拍 preset(ADR-015 v0.3)同验通过:俯仰勾选行隐藏、三段脚本、手动推进、产物 `roam-patch-*.webm`。**附带修复 HUD 重叠**:v4 加「下一段」按钮(bottom≈92px)时压住了覆盖行(120px)/计时行(90px),三行挤叠;覆盖/计时改 safe-area 基准整体上移(+158px/+128px),底部自下而上「停止→下一段→REC→覆盖」四层不重叠 |
